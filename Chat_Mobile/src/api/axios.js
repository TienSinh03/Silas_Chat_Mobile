...
const HOST_IP = '192.168.10.188'; // nhập ipconfig trên cmd để lấy địa chỉ ipv4
...
instance.interceptors.response.use(
   ...
   async error => {
        ...
        if (error.response.status === 401 && !originalRequest._retry &&
            !(originalRequest.url.includes("/api/v1/auth/refresh-token") || originalRequest.url.includes("/api/v1/auth/sign-in"))
        ) {
            ...
            if(refreshToken) {
                try {
                    const response = await axios.post(`${chatEndpoint}/api/v1/auth/refresh-token`, { refreshToken: refreshToken });
