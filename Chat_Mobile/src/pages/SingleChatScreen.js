...
const SingleChatScreen = ({ navigation, route }) => {
  ...
  const { receivedFriendRequests, sentRequests } = useSelector(
    (state) => state.friend
  );
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      // Nếu chọn nhiều ảnh, result.assets sẽ là một mảng
  ...
  useEffect(() => {
    ...
    if (userReceived?.id) {
      ...
      const isSent = requestsSent.find(
        (req) => req?.userId === userReceived?.id
      );
      if (isSent) {
        setIsSentReq(true);
      } else {
